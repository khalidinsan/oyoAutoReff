...
function Register($phoneNumber,$countryCode){
	$ch = curl_init();

	curl_setopt($ch, CURLOPT_URL, 'https://api.oyorooms.com/v2/users/generate_otp?phone='.$phoneNumber.'&nod=4&intent=login&sms_auto_retrieval=true&country_code=%2B'.$countryCode.'&version=20233&partner_app_version=20233&android_id='.generateRandomString(16).'&idfa=&sid=1551940465205');
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
...
function verifRegister($phoneNumber,$reffCode,$otp){
	$randName = randName();
	$email = str_replace(" ", "", $randName) . mt_rand(100, 999);
	$ch = curl_init();
...
echo 'Masukkan Kode Refferal : '; 
$reffCode = trim(fgets(STDIN)); 
$ulang = "Y";
while($ulang=="Y"||$ulang=="y"){
	echo "-----------------------------------------\n";
	echo 'Masukkan Kode Negara (62/1) : '; 
	$countryCode = trim(fgets(STDIN)); 
	echo 'Masukkan Nomor HP : '; 
	$phoneNumber = trim(fgets(STDIN)); 
	$register = Register($phoneNumber,$countryCode);
	if($register->is_user_present == 1){
		echo "Nomor sudah terdaftar";
	}else{
		echo 'Masukkan OTP : '; 
		$otp = trim(fgets(STDIN)); 
		$verifRegister = verifRegister($phoneNumber,$reffCode,$otp);
		if(isset($verifRegister->user_id)){
			echo "Berhasil mendaftarkan akun\n";
		}else{
			echo "Terjadi kesalahan \n";
		}

	}
	echo "[?] Apakah ingin mengulangnya lagi ? (Y/N) : ";
